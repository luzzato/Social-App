...
public class UsersDatabaseManager {
    ...
    public static User getUserFromCursor(Cursor cursor) {
        ...
        String externalid = cursor.getString(cursor.getColumnIndex(UserTable.Cols.EXTERNAL_ID));//newly add
        boolean online = cursor.getInt(cursor.getColumnIndex(UserTable.Cols.IS_ONLINE)) > 0;

        User user = new User(id, fullName, email, phone, avatarUid, externalid);//externalid add
